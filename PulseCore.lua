...
settingsTab = createTabButton("SettingsTab", "SETTINGS", 372)
...
do
    local featuresInfo = create("TextLabel", {
        ...
        BorderSizePixel = 0,
        Text = "MAIN FEATURES\n• WalkSpeed / Velocity speed methods with Increase and Set calculation modes\n• Optional AssemblyLinearVelocity Jump Boost for Local and every Ability\n• Up to four custom Abilities with delay, duration, cooldown, and hotkeys\n• Survivor / Executioner ESP and boost-status Tabs overlay\n• Auto Select, Fun quick commands, Inf Flight, Noclip, Infinity Jump, live console mode, and session configs",
        Font = Enum.Font.GothamMedium,
        TextSize = 13,
        TextColor3 = COLORS.MutedText,
        TextWrapped = true,
...
do
    local changelogText = table.concat({
        ...
        "• Added WalkSpeed and horizontal Velocity methods plus Increase and Set calculation modes for Standard Boost and every Ability.",
...
function selectTab(tabName)
    ...
    local pageTitles = {
        ...
        Fun = { "FUN", "Quick commands and animation controls" },
...
function configManager.loadConfigByName(configName, options)
    ...
    clientModules.boostTabs.setEnabled(visuals.tabs == true, true)
...
clientModules.speedControl.standardMethodButton.Activated:Connect(function()
    clientModules.speedControl.standardMethod =
        clientModules.speedControl.standardMethod == "WalkSpeed" and "Velocity" or "WalkSpeed"
    clientModules.speedControl.standardMethodButton.Text =
        clientModules.speedControl.getMethodLabel(clientModules.speedControl.standardMethod)
end)
...
clientModules.inventoryOrder.toggleButton.Activated:Connect(function()
    clientModules.inventoryOrder.setEnabled(
        not clientModules.inventoryOrder.enabled,
        false
    )
end)
...
clientModules.boostTabs.toggleButton.Activated:Connect(function()
    clientModules.boostTabs.setEnabled(not clientModules.boostTabs.enabled, false)
end)
