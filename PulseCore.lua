...
UPDATE_CHECK_FINGERPRINT = "4268D1309F4D5301F58FE9152452A8FC"
...
-- Responsive device scaling: smaller UI on phones and tablets, full size on desktop.
local function updatePulseCoreDeviceScale()
    ...
    if touchDevice then
        if width <= 700 or height <= 500 then
            -- Phone
            scale = 0.68
        else
            -- Tablet / large touch display
            scale = 0.82
        end
    elseif width < 900 or height < 650 then
...
local deviceScaleConnection = nil
pcall(function()
    deviceScaleConnection = workspace:GetPropertyChangedSignal("CurrentCamera"):Connect(function()
        task.defer(updatePulseCoreDeviceScale)
    end)
end)

task.spawn(function()
    while not guiDestroyed and screenGui and screenGui.Parent do
        local camera = workspace.CurrentCamera
        if camera then
            pcall(updatePulseCoreDeviceScale)
        end
        task.wait(1)
    end
end)
...
clientModules = {
    ...
    customLms = {
        ...
        activeCharacter = nil,
        activeAsset = nil,
        standardSounds = setmetatable({}, { __mode = "k" }),
        interceptedStandard = {},
        updateConnection = nil,
...
clientModules.tabs.custom = createTabButton("CustomTab", "CUSTOM", 170)
clientModules.tabs.combat = createTabButton("CombatTab", "COMBAT", 222)
clientModules.tabs.combat.Visible = false
clientModules.tabs.fun = createTabButton("FunTab", "FUN", 274)
clientModules.tabs.fun.Visible = false
clientModules.tabs.performance = createTabButton("PerformanceTab", "PERFORMANCE", 326)
clientModules.tabs.autoSelect = createTabButton("AutoSelectTab", "AUTO", 378)
clientModules.tabs.keyList = createTabButton("KeyListTab", "KEY LIST", 430)
settingsTab = createTabButton("SettingsTab", "SETTINGS", 482)
...
clientModules.pages.custom = createScrollingPage("CustomPage")
...
clientModules.pages.custom.Visible = false
...
createSectionLabel(clientModules.pages.custom, "CUSTOM / LMS", 1)

clientModules.customLms.toggleButton, clientModules.customLms.toggleDot =
    createToggleRow(clientModules.pages.custom, "Custom LMS", 2)

clientModules.customLms.statusLabel = create("TextLabel", {
    LayoutOrder = 3,
    Size = UDim2.new(1, 0, 0, 72),
    BackgroundColor3 = COLORS.CyanDeep,
    BackgroundTransparency = 0.32,
    BorderSizePixel = 0,
    Text = "Custom LMS is disabled. Create the character folders and place audio files inside them.",
    Font = Enum.Font.GothamMedium,
    TextSize = 12,
    TextColor3 = COLORS.MutedText,
    TextWrapped = true,
...
addCorner(clientModules.customLms.statusLabel, 12)
addStroke(clientModules.customLms.statusLabel, COLORS.Border, 0.28, 1)
create("UIPadding", {
    PaddingLeft = UDim.new(0, 14),
    PaddingRight = UDim.new(0, 14),
}, clientModules.customLms.statusLabel)

local customCharacterTabsFrame = create("Frame", {
    LayoutOrder = 4,
    Size = UDim2.new(1, 0, 0, 112),
    BackgroundTransparency = 1,
    BorderSizePixel = 0,
...
local customCharacterTabsLayout = create("UIGridLayout", {
    CellSize = UDim2.fromOffset(116, 34),
    CellPadding = UDim2.fromOffset(8, 6),
    FillDirection = Enum.FillDirection.Horizontal,
    FillDirectionMaxCells = 5,
    SortOrder = Enum.SortOrder.LayoutOrder,
}, customCharacterTabsFrame)

local function customLmsSetCharacterVisual(character)
    for name, button in pairs(clientModules.customLms.characterButtons) do
        if button and button.Parent then
            local active = name == clientModules.customLms.currentCharacter
            button.BackgroundColor3 = active and COLORS.CyanDark or COLORS.Input
            button.TextColor3 = active and COLORS.Text or COLORS.MutedText
        end
    end
end

local function customLmsMakeCharacterButton(character, index)
    local button = create("TextButton", {
        Name = string.gsub(character, "[^%w]", "") .. "CustomTab",
        LayoutOrder = index,
        BackgroundColor3 = COLORS.Input,
    ...
    addCorner(button, 9)
    addStroke(button, COLORS.Border, 0.35, 1)
    clientModules.customLms.characterButtons[character] = button

    button.Activated:Connect(function()
        clientModules.customLms.currentCharacter = character
        customLmsSetCharacterVisual(character)
        if clientModules.customLms.refreshTrackList then
            clientModules.customLms.refreshTrackList(character)
        end
    end)
end

for index, character in ipairs(CUSTOM_LMS_CHARACTERS) do
    customLmsMakeCharacterButton(character, index)
end
customLmsSetCharacterVisual(clientModules.customLms.currentCharacter)

clientModules.customLms.refreshButton = create("TextButton", {
    LayoutOrder = 5,
    Size = UDim2.new(1, 0, 0, 38),
    BackgroundColor3 = COLORS.CyanDark,
    BackgroundTransparency = 0.11,
...
addCorner(clientModules.customLms.refreshButton, 12)
addStroke(clientModules.customLms.refreshButton, COLORS.Cyan, 0.35, 1)

clientModules.customLms.trackListFrame = create("ScrollingFrame", {
    LayoutOrder = 6,
...
addCorner(clientModules.customLms.trackListFrame, 12)
addStroke(clientModules.customLms.trackListFrame, COLORS.Border, 0.30, 1)
create("UIPadding", {
    PaddingTop = UDim.new(0, 8),
    PaddingBottom = UDim.new(0, 8),
    PaddingLeft = UDim.new(0, 8),
    PaddingRight = UDim.new(0, 8),
}, clientModules.customLms.trackListFrame)
local customTrackListLayout = create("UIListLayout", {
    Padding = UDim.new(0, 7),
    SortOrder = Enum.SortOrder.LayoutOrder,
}, clientModules.customLms.trackListFrame)

function clientModules.customLms.setStatus(text, color)
    if clientModules.customLms.statusLabel and clientModules.customLms.statusLabel.Parent then
        clientModules.customLms.statusLabel.Text = text
        clientModules.customLms.statusLabel.TextColor3 = color or COLORS.MutedText
    end
end

function clientModules.customLms.normalizeCharacter(value)
    local normalized = string.lower(tostring(value or ""))
    normalized = string.gsub(normalized, "[%s_%-%.]", "")
    return normalized
end

function clientModules.customLms.getFolderPath(character)
    return CUSTOM_LMS_ROOT .. "\\" .. tostring(character)
end

function clientModules.customLms.ensureFolders()
    local makeFolderApi = getPulseCoreFileApi("makefolder")
    local isFolderApi = getPulseCoreFileApi("isfolder")

    if not makeFolderApi then
        clientModules.customLms.setStatus("Custom LMS: file API makefolder is unavailable.", COLORS.Red)
        return false
    end

    local function ensureFolder(path)
        if isFolderApi then
            local okExists, exists = pcall(isFolderApi, path)
            if okExists and exists == true then
                return true
            end
        end

        local okCreate = pcall(makeFolderApi, path)
        if okCreate then
            return true
        end
    ...
    if not ensureFolder("PulseCore") then
        clientModules.customLms.setStatus("Custom LMS: could not create PulseCore folder.", COLORS.Red)
        return false
    end

    if not ensureFolder(CUSTOM_LMS_ROOT) then
        clientModules.customLms.setStatus("Custom LMS: could not create PulseCore\\Custom LMS.", COLORS.Red)
        return false
    end

    for _, character in ipairs(CUSTOM_LMS_CHARACTERS) do
        local folderPath = clientModules.customLms.getFolderPath(character)
        if not ensureFolder(folderPath) then
            clientModules.customLms.setStatus(
                "Custom LMS: could not create folder for " .. character .. ".",
                COLORS.Red
            )
            return false
        end
    ...
    clientModules.customLms.foldersReady = true
    return true
end

function clientModules.customLms.getFileName(path)
    local name = tostring(path or ""):match("([^\\/]+)$") or tostring(path or "")
    return name ~= "" and name or "Track"
end

function clientModules.customLms.getTrackDisplayName(path)
    return clientModules.customLms.getFileName(path):gsub("%.[^%.]+$", "")
end

function clientModules.customLms.isAudioPath(path)
    local extension = tostring(path or ""):lower():match("%.([%w]+)$")
    return extension ~= nil and CUSTOM_LMS_AUDIO_EXTENSIONS[extension] == true
end

function clientModules.customLms.listTracks(character)
    local listFilesApi = getPulseCoreFileApi("listfiles")
    local isFileApi = getPulseCoreFileApi("isfile")
    local folderPath = clientModules.customLms.getFolderPath(character)

    if not listFilesApi then
        return {}
    end

    local ok, paths = pcall(listFilesApi, folderPath)
    ...
    for _, path in ipairs(paths) do
        if type(path) == "string" and clientModules.customLms.isAudioPath(path) then
            local validFile = true
            if isFileApi then
                local okFile, isFile = pcall(isFileApi, path)
                validFile = okFile and isFile == true
            end
            if validFile then
    ...
    table.sort(result, function(a, b)
        return string.lower(clientModules.customLms.getFileName(a)) < string.lower(clientModules.customLms.getFileName(b))
    end)

    return result
end

function clientModules.customLms.updateToggleVisual()
    if clientModules.customLms.toggleButton and clientModules.customLms.toggleDot then
        setSwitchVisual(
            clientModules.customLms.toggleButton,
            clientModules.customLms.toggleDot,
            clientModules.customLms.enabled == true
        )
    end
end

function clientModules.customLms.stopCurrentSound(resumeStandard)
    local sound = clientModules.customLms.sound
    if sound then
        pcall(function()
            sound:Stop()
        end)
    ...
    clientModules.customLms.activePath = nil
    clientModules.customLms.activeCharacter = nil
    clientModules.customLms.activeAsset = nil

    if resumeStandard then
        clientModules.customLms.resumeStandardTracks()
    end
end

function clientModules.customLms.isSoloThemeSound(sound)
    ...
    while ancestor do
        if ancestor:IsA("Folder") and ancestor.Name == "SoloTheme" then
            return true
        end
...
function clientModules.customLms.isStandardLmsSound(sound)
    if not sound or not sound:IsA("Sound") then
        return false
    end

    if not clientModules.customLms.isSoloThemeSound(sound) then
        return false
    end

    if CUSTOM_LMS_STANDARD_SOUND_NAMES[sound.Name] then
        return true
    end

    local soundId = tostring(sound.SoundId or ""):match("%d+")
    return soundId ~= nil and CUSTOM_LMS_STANDARD_SOUND_IDS[soundId] == true
end

function clientModules.customLms.registerStandardSound(instance)
    if instance and instance:IsA("Sound") and clientModules.customLms.isStandardLmsSound(instance) then
        clientModules.customLms.standardSounds[instance] = true
    end
end

function clientModules.customLms.stopStandardTracks()
    for sound in pairs(clientModules.customLms.standardSounds) do
        if sound and sound.Parent and clientModules.customLms.isStandardLmsSound(sound) then
            local isPlaying = false
            local volume = 1

            pcall(function()
                isPlaying = sound.IsPlaying == true
                volume = tonumber(sound.Volume) or 1
            end)

            if isPlaying then
                if not clientModules.customLms.interceptedStandard[sound] then
                    clientModules.customLms.interceptedStandard[sound] = {
                        volume = volume,
                    }
                end
...
function clientModules.customLms.resumeStandardTracks()
    for sound, record in pairs(clientModules.customLms.interceptedStandard) do
        if sound and sound.Parent and clientModules.customLms.isStandardLmsSound(sound) then
            pcall(function()
                sound.Volume = tonumber(record and record.volume) or 1
            end)
        end

        clientModules.customLms.interceptedStandard[sound] = nil
    end
end

function clientModules.customLms.getActiveStandardLmsSound()
    for sound in pairs(clientModules.customLms.standardSounds) do
        if sound and sound.Parent and clientModules.customLms.isStandardLmsSound(sound) then
            local isPlaying = false
            pcall(function()
...
function clientModules.customLms.playTrack(character, path)
    if not character or not path then
        return false
    end

    if not clientModules.customLms.sound then
        local soundService = game:GetService("SoundService")
        local sound = Instance.new("Sound")
        sound.Name = "PulseCoreCustomLMS"
        sound.Looped = true
        sound.Volume = 1
        sound.PlayOnRemove = false
        sound.Parent = soundService
        clientModules.customLms.sound = sound
    end

    local asset = getPulseCoreCustomAsset(path)
    if not asset then
        clientModules.customLms.setStatus(
            "Custom LMS: this track could not be loaded by the current executor: "
                .. clientModules.customLms.getFileName(path),
            COLORS.Red
        )
        return false
    end
    ...
    clientModules.customLms.activePath = path
    clientModules.customLms.activeCharacter = character
    clientModules.customLms.activeAsset = asset
    return sound.IsPlaying == true
end

function clientModules.customLms.refreshTrackList(character)
    character = character or clientModules.customLms.currentCharacter
    clientModules.customLms.currentCharacter = character

    if not clientModules.customLms.trackListFrame then
        return
    end

    for _, child in ipairs(clientModules.customLms.trackListFrame:GetChildren()) do
        if child:IsA("TextButton") or (child:IsA("TextLabel") and child.Name == "CustomLMSEmpty") then
            child:Destroy()
        end
    end

    local tracks = clientModules.customLms.listTracks(character)
    clientModules.customLms.trackLists[character] = tracks

    local selectedPath = clientModules.customLms.selectedTracks[character]
    ...
    if not selectedStillExists then
        clientModules.customLms.selectedTracks[character] = nil
        selectedPath = nil
    end

    for index, path in ipairs(tracks) do
        local selected = path == selectedPath
        local button = create("TextButton", {
            Name = "CustomLMSTrack" .. index,
            LayoutOrder = index,
            Size = UDim2.new(1, -8, 0, 38),
            BackgroundColor3 = selected and COLORS.Green or COLORS.CyanDark,
            BackgroundTransparency = selected and 0.04 or 0.11,
            BorderSizePixel = 0,
            Text = (selected and "SELECTED  •  " or "") .. clientModules.customLms.getTrackDisplayName(path),
            Font = Enum.Font.GothamBold,
            TextSize = 11,
            TextColor3 = selected and COLORS.CyanDeep or COLORS.Text,
            AutoButtonColor = true,
            Active = true,
            Selectable = true,
        }, clientModules.customLms.trackListFrame)
        addCorner(button, 10)
        addStroke(button, COLORS.Cyan, 0.40, 1)

        button.Activated:Connect(function()
            clientModules.customLms.selectedTracks[character] = path
            clientModules.customLms.refreshTrackList(character)
            if clientModules.customLms.enabled then
                clientModules.customLms.update()
            end
        end)
    end
    ...
    if #tracks == 0 then
        local empty = create("TextLabel", {
            ...
            BackgroundTransparency = 1,
            Text = "No audio files found.\nPut .mp3, .ogg or .wav files into:\nPulseCore\\Custom LMS\\" .. character,
            Font = Enum.Font.GothamMedium,
            TextSize = 11,
            TextColor3 = COLORS.MutedText,
            TextWrapped = true,
...
function clientModules.customLms.update()
    if not clientModules.customLms.enabled then
        clientModules.customLms.stopCurrentSound(true)
        return
    end

    local activeStandardLmsSound = clientModules.customLms.getActiveStandardLmsSound()
    if not activeStandardLmsSound then
        clientModules.customLms.stopCurrentSound(true)
        clientModules.customLms.setStatus(
            "Custom LMS enabled. Waiting for the game's SoloTheme LMS music to start.",
            COLORS.MutedText
        )
        return
    end

    -- The game itself determines when LMS starts. The selected Custom LMS
    -- profile is independent from the character currently played by the local
    -- player, so only the active SoloTheme sound is used as the LMS signal.
    local character = clientModules.customLms.currentCharacter
    if not character then
        character = CUSTOM_LMS_CHARACTERS[1]
        clientModules.customLms.currentCharacter = character
    end
    ...
    if not selectedPath then
        clientModules.customLms.stopCurrentSound(true)
        clientModules.customLms.setStatus(
            "1v1 detected as " .. detectedCharacter
                .. ", but no Custom LMS track is selected for " .. character .. ".",
            COLORS.Yellow
        )
        return
    end

    clientModules.customLms.stopStandardTracks()

    local sound = clientModules.customLms.sound
    if clientModules.customLms.activePath ~= selectedPath
        or clientModules.customLms.activeCharacter ~= character
        or not sound
        or not sound.Parent
        or not sound.IsPlaying
    then
        local played = clientModules.customLms.playTrack(character, selectedPath)
        if not played then
            return
        end
    end
    ...
    clientModules.customLms.setStatus(
        "Custom LMS active: " .. character
            .. "  •  game track: " .. tostring(activeStandardLmsSound.Name)
            .. "  •  " .. clientModules.customLms.getFileName(selectedPath),
        COLORS.Green
    )
end

function clientModules.customLms.refreshAll()
    if not clientModules.customLms.ensureFolders() then
        return
    end

    for _, character in ipairs(CUSTOM_LMS_CHARACTERS) do
        clientModules.customLms.trackLists[character] = clientModules.customLms.listTracks(character)
    end

    clientModules.customLms.refreshTrackList(clientModules.customLms.currentCharacter)
    clientModules.customLms.setStatus(
        "Folders ready. Select one track per character. The selected character profile can be used even when you are playing a different character.",
        COLORS.MutedText
    )
end

function clientModules.customLms.setEnabled(enabled, silent)
    clientModules.customLms.enabled = enabled == true
    clientModules.customLms.updateToggleVisual()

    if clientModules.customLms.enabled then
        if not clientModules.customLms.foldersReady then
            clientModules.customLms.refreshAll()
        else
            clientModules.customLms.refreshTrackList(clientModules.customLms.currentCharacter)
        end
        ...
        if not silent then
            clientModules.customLms.setStatus(
                "Custom LMS enabled. Waiting for a 1v1.",
                COLORS.Green
            )
        end
    else
...
function clientModules.customLms.initialize()
    if clientModules.customLms.initialized then
        return
    end

    clientModules.customLms.initialized = true

    for _, instance in ipairs(game:GetDescendants()) do
        clientModules.customLms.registerStandardSound(instance)
    end

    clientModules.customLms.descendantConnection = game.DescendantAdded:Connect(function(instance)
        clientModules.customLms.registerStandardSound(instance)
    end)
    ...
    if clientModules.customLms.refreshButton then
        clientModules.customLms.refreshButton.Activated:Connect(function()
            clientModules.customLms.refreshAll()
        end)
    end

    if clientModules.customLms.toggleButton then
        clientModules.customLms.toggleButton.Activated:Connect(function()
            clientModules.customLms.setEnabled(
                not clientModules.customLms.enabled,
                false
            )
        end)
    end

    clientModules.customLms.updateConnection = RunService.Heartbeat:Connect(function(deltaTime)
        if guiDestroyed then
            return
        end

        clientModules.customLms.accumulator = clientModules.customLms.accumulator + deltaTime
        if clientModules.customLms.accumulator < 0.08 then
            return
        end
        clientModules.customLms.accumulator = 0
        clientModules.customLms.update()
    end)
end
...
visualsHint = create("TextLabel", {
    ...
    BackgroundTransparency = 0.32,
    BorderSizePixel = 0,
    Text = "ESP detects recognized characters by ability names. Survivors use blue; Executioners use scarlet. A name tag stays above each highlighted character and scales down with distance.",
    Font = Enum.Font.GothamMedium,
    TextSize = 12,
    TextColor3 = COLORS.MutedText,
    TextWrapped = true,
...
function selectTab(tabName)
    local pageByName = {
        ...
        Local = localPage,
        Visuals = visualsPage,
        Custom = clientModules.pages.custom,
    ...
    local buttonByName = {
        ...
        Local = localTab,
        Visuals = visualsTab,
        Custom = clientModules.tabs.custom,
    ...
    local pageTitles = {
        ...
        Custom = { "CUSTOM", "Character-specific Custom LMS audio" },
    ...
    setTabSelected(clientModules.tabs.custom, tabName == "Custom", instant)
...
function refreshSpecialTabsVisibility()
    clientModules.specialTabsAllowed = true
    if clientModules.tabs.combat then
        clientModules.tabs.combat.Visible = false
    end
    if clientModules.tabs.fun then
        clientModules.tabs.fun.Visible = false
    end
    relayoutSidebarTabs()
    clientModules.pages.combat.Visible = false
...
clientModules.tabs.custom.Activated:Connect(function()
    selectTab("Custom")
end)
